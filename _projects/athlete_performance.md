---
layout: page
title: Athlete Performance
description: Wearable technology for monitoring and enhancing athletic performance
img: assets/img/athlete_performance.png
importance: 3
category: Active
---



Athletic performance measures are critical for understanding an athlete's explosive power and strength. Traditional methods for calculating metrics like the Reactive Strength Index (RSI) require expensive force plates that are stationary and difficult to deploy in field settings. Our research explores using low-cost, lightweight inertial measurement units (IMUs) as alternative for measuring athlete performance.


### Reactive Strength Index Research

Undergraduate researcher Stephen Mitchell led a multi-year effort to validate IMU-based measurements of the Drop Jump Ratio, a key component of calculating RSI. His work demonstrated that low-cost wearable sensors can effectively capture the kinematic data needed to assess athletic performance without the limitations of traditional force plates.

 <div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid path="/assets/img/stephen-sieds-2021.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="caption">
    Stephen Mitchell presenting his RSI research results at SIEDS 2021
</div>

Stephen's research was presented at multiple venues:
- **MARCUS 2019**: Initial findings on IMU-based drop jump measurements ([abstract](/assets/pdf/mitchell-marcus-abstract.pdf), [poster](/assets/pdf/mitchell-marcus-poster.pdf))
- **IEEE SouthEast Conference 2021**: Extended analysis ([short paper](/assets/pdf/MitchellS-T7S2-ieee-ready.pdf), [presentation](/assets/pdf/Mitchell-IEEE-Presentation.pdf))
- **SIEDS 2021**: Full publication with full user study ([full paper](/assets/pdf/mitchell-sieds-camera-ready.pdf))


This work was conducted in collaboration with [Dr. Stu Thompson](http://www.bucknell.edu/fac-staff/stu-thompson) (Bucknell University, Electrical and Computer Engineering) and [Dr. Rosha Wunderlich](https://www.jmu.edu/biology/people/current-people/faculty/faculty-wunderlich.shtml) (JMU Biology).

## JMU Softball Collaboration

*Content coming soon - collaboration with JMU Softball program on wearable performance monitoring.*

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        <video width="100%" controls muted class="img-fluid rounded z-depth-1">
            <source src="{{ 'assets/video/jmu-softball.m4v' | relative_url }}" type="video/mp4">
            Your browser does not support the video tag.
        </video>
    </div>
</div>
<div class="caption">
    JMU Softball in the Wearable Computing Lab
</div>
